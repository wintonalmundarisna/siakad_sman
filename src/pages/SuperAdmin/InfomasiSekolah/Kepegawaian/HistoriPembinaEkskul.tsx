...
const HistoriPembinaEkskul = () => {
  ...
  const [data, setData] = useState<PembinaData | null>(null);
  ...
  useEffect(() => {
    if (!id) return;
    api
      .get(`/spa/pembina/ekstrakurikuler/${id}`)
      .then((res) => {
        if (res.data.status === "success" && res.data.data) {
          const d: PembinaData = res.data.data;
          setData(d);
        ...
        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              {data && <p className="text-sm text-muted-foreground mt-0.5">{data.nama_pembina}</p>}
            </div>
          </div>
                              ...
                              {periode.ekstrakurikuler.map((ekskul) => (
                                <div key={ekskul.ekskul_id} className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
                                  ...
                                  <div className="flex gap-2">
                                    <Badge className="bg-blue-100 text-blue-700 text-xs">{ekskul.status_ekskul}</Badge>
