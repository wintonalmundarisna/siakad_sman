...
const UbahPasswordKepegawaian = () => {
  ...
  // ── Fetch email pegawai dari detail ──────────────────────────────────────
  useEffect(() => {
    const fetchPegawai = async () => {
      try {
        setLoadingPegawai(true);
        const res = await api.get(`/spa/kepegawaian/${id}`);
        if (res.data.status === "success") {
          const d = res.data.data;
          setNamaPegawai(d.nama ?? "");
        ...
        Swal.fire({
          ...
          text: err.response?.data?.message || "Tidak dapat memuat data pegawai.",
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("/spa/ubah-password/kepegawaian", {
      ...
      if (res.data.status === "success") {
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
          text: "Password pegawai berhasil diperbarui.",
  ...
  return (
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[300px]"}`}>
        <PageTitle title="Ubah Password Kepegawaian" />
        ...
        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-3xl font-bold">Ubah Password Kepegawaian</h1>
          </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base">
                    ...
                    Ubah Password — {namaPegawai}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Email (readonly, dari data pegawai) */}
                    <div>
                      <label className="block text-sm font-semibold mb-1">Email Pegawai</label>
                      <input type="email" value={formData.email} readOnly className="border p-2 w-full rounded bg-gray-50 text-gray-500 text-sm cursor-not-allowed" />
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        />
                        <button type="button" onClick={() => setShowLama(!showLama)} className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="border p-2 w-full rounded pr-10 text-sm"
                        />
                        <button type="button" onClick={() => setShowBaru(!showBaru)} className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="border p-2 w-full rounded pr-10 text-sm"
                        />
                        <button type="button" onClick={() => setShowKonfirmasi(!showKonfirmasi)} className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600">
...
export default UbahPasswordKepegawaian;
