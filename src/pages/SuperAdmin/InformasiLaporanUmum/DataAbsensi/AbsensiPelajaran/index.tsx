 ...
 * Route: /superadmin/informasi-laporan-umum/absensi-pelajaran
 *
 * Backend:
 *  GET    /spa/absensi/guru/pelajaran/data-select               → filter options
 *  GET    /spa/guru/absensi/pelajaran?tahun_akademik_id=X&semester_id=Y → list
 *  PUT    /spa/guru/absensi/pelajaran/:id                       → update status
 *  DELETE /spa/absensi/guru/pelajaran/destroy?ids[]=X           → delete bulk
 *  GET    /spa/absensi/guru/pelajaran/export?ids[]=X            → export excel
 */
...
const DataAbsensiPelajaran = () => {
  ...
  useEffect(() => {
    ...
    api
      .get("/spa/guru/absensi/pelajaran", {
        params: {
  ...
  const stats = useMemo(() => {
    ...
    const totalGuru = new Set(dataFlat.map((x) => x.guru_id)).size;
    return { hadir, tidakHadir, persen, totalGuru };
  }, [dataFlat]);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      await api.put(`/spa/guru/absensi/pelajaran/${editingId}`, { status: editStatus });
      setEditDialog(false);
  ...
  const handleDeleteMultiple = async () => {
    ...
    try {
      setLoadingData(true);
      await api.delete("/spa/absensi/guru/pelajaran/destroy", { params: { ids: deletableIds } });
      setSelectedIds([]);
  ...
  const handleExport = async (selected = false) => {
    ...
    try {
      const res = await api.get("/spa/absensi/guru/pelajaran/export", {
        params: {
          ids,
    ...
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[300px]"}`}>
        ...
        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          <div className="mb-6">
            <h1 className="text-3xl font-bold">Data Absensi Pelajaran Guru</h1>
            <p className="text-sm text-muted-foreground mt-1">Kelola data absensi pelajaran guru per tahun akademik dan semester</p>
          </div>
            ...
            <>
              ...
              {!loadingData && dataFlat.length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                  {[
                    { label: "Total Guru", value: stats.totalGuru, icon: UserCheck, color: "bg-blue-50 text-blue-700 border-blue-200" },
                            ...
                            <button type="button" className="w-full bg-primary px-5 py-4 flex items-center justify-between hover:bg-primary/90 transition-colors" onClick={() => toggleSemester(semGroup.semester_id_key)}>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="text-right">
                                  <p className="text-white font-semibold">{semGroup.gurus.length} guru</p>
        ...
        <Dialog open={editDialog} onOpenChange={setEditDialog}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Ubah status kehadiran guru pada pelajaran</DialogDescription>
            </DialogHeader>
            {editingRow && (
