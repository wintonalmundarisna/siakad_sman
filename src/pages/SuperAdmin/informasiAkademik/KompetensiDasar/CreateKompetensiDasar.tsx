...
const CreateKompetensi = () => {
  ...
  // ✅ kurmapIdParam dan kurikulumIdParam dulu
  ...
  const [formData, setFormData] = useState<FormData>({
    kurikulum_mata_pelajaran_id: kurmapIdParam,
    judul_kompetensi: "",
    jenis: "",
    kode: "",
    tingkat: "",
  ...
  // ✅ Baru derive selectedKurmap dan isMerdeka (setelah formData ada)
  ...
  const buildBackUrl = () => {
    if (kurmapIdParam && kurikulumIdParam) {
      return `/superadmin/informasi-sekolah/kompetensi?kurikulum_mata_pelajaran_id=${kurmapIdParam}&kurikulum_id=${kurikulumIdParam}`;
    }
    if (kurmapIdParam) {
      return `/superadmin/informasi-sekolah/kompetensi?kurikulum_mata_pelajaran_id=${kurmapIdParam}`;
    }
    return "/superadmin/informasi-sekolah/kompetensi";
  };

  // Fetch select
  useEffect(() => {
    api
      .get("/spa/data-select/kompetensi")
      .then((res) => {
        const d = res.data?.data;
        if (d && Array.isArray(d.kurikulum_mata_pelajaran)) {
          setKurmapList(d.kurikulum_mata_pelajaran);

          // Validasi: apakah kurmapIdParam ada di list?
          if (kurmapIdParam) {
            const exists = d.kurikulum_mata_pelajaran.some((k: KurmapOption) => String(k.kurikulum_mata_pelajaran_id) === kurmapIdParam);
            if (!exists) {
              // ID tidak ditemukan di list — reset supaya user pilih manual
              setFormData((prev) => ({ ...prev, kurikulum_mata_pelajaran_id: "" }));
              setErrors((prev) => ({
                ...prev,
                kurikulum_mata_pelajaran_id: ["Mata pelajaran tidak ditemukan di daftar aktif. Silakan pilih ulang."],
              }));
            }
          }
        }
  ...
  // ✅ Tambahkan useEffect reset jenis DI SINI — setelah useEffect fetch
  useEffect(() => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const successUrl = selectedKurmapId ? `/superadmin/informasi-sekolah/kompetensi?kurikulum_mata_pelajaran_id=${selectedKurmapId}&kurikulum_id=${kurikulumIdParam}` : "/superadmin/informasi-sekolah/kompetensi";
                ...
                <Select value={formData.jenis} onValueChange={handleJenisChange}>
                  ...
                  <SelectContent>
                    <SelectGroup>
                      ...
                      {/* Belum pilih kurmap → tampilkan semua */}
                      {!selectedKurmap && (
                        <>
                          <SelectItem value="KD">KD — Kompetensi Dasar</SelectItem>
                          <SelectItem value="CP">CP — Capaian Pembelajaran</SelectItem>
                        </>
                      )}
                      {/* Sudah pilih kurmap → filter sesuai tipe */}
