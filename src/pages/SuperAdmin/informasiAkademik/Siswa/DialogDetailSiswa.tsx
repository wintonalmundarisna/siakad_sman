    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Informasi data siswa dan tautan ke histori terkait.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-5 py-2">
            ...
            <Card>
              ...
              <CardContent className="flex flex-col gap-3">
                ...
                {/* Semua siswa */}
                <LinkButton
                  to={`/superadmin/informasi-sekolah/siswa-rombel?siswa_id=${siswaId}`}
                  icon={<GraduationCap size={15} />}
                  label="Histori Rombel"
                  onClick={() => setOpen(false)}
                />
                <LinkButton
                  to={`/superadmin/informasi-akademik/jadwal-pelajaran-siswa?siswa_id=${siswaId}`}
                  icon={<BookOpen size={15} />}
                  label="Histori Jadwal Pelajaran"
                  onClick={() => setOpen(false)}
                />
                <LinkButton
                  to={`/superadmin/informasi-laporan-umum/absensi-siswa?siswa_id=${siswaId}`}
                  icon={<ClipboardList size={15} />}
                  label="Histori Absensi Siswa - Pelajaran"
                  onClick={() => setOpen(false)}
                />
                <LinkButton
                  to={`/superadmin/informasi-akademik/siswa-ekskul?siswa_id=${siswaId}`}
                  icon={<Trophy size={15} />}
                  label="Histori Ekstrakurikuler"
                  onClick={() => setOpen(false)}
                />
                <LinkButton
                  to={`/superadmin/informasi-akademik/prestasi-siswa?siswa_id=${siswaId}`}
                  icon={<Star size={15} />}
                  label="Histori Prestasi"
                  onClick={() => setOpen(false)}
                />
                <LinkButton
            ...
            <Card>
              ...
              <CardContent>
                <LinkButton
                  to={`/superadmin/informasi-akademik/siswa/ubah-password/${siswaId}`}
